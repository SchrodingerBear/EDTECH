...
function url(string $path = '', bool $absolute = false): string
{
    ...
    // Always use BASE_URL to prevent ../ issues and avoid root-relative / paths
...
function org_url(string $slug, string $sub = ''): string
{
    ...
    $cleanSub = trim($sub, '/\\');
    if ($cleanSub === '') {
        return url('organizations/index.php?org=' . urlencode($cleanSlug));
    }
    
    $path = 'organizations/' . $cleanSlug . '/' . $cleanSub;
    return url($path);
}
...
function ai_generate_description(string $name, string $institution = '', array|string $info = []): string
{
    ...
    if (file_exists(__DIR__ . '/ollama-wrapper.php')) {
        require_once __DIR__ . '/ollama-wrapper.php';
        $ai = ai_text();
        if ($ai !== null) {
            try {
                ...
                if (!empty($info['facilities'])) {
                    $facilities = is_array($info['facilities']) ? implode(', ', $info['facilities']) : $info['facilities'];
                    $details['facilities'] = $facilities;
                }
                if (!empty($info['purpose'])) {
                    $details['purpose'] = $info['purpose'];
                }
                if (!empty($info['details'])) {
                    $details['details'] = $info['details'];
                }
                
                $result = $ai->generateCampusDescription($name, $institution, $details);
    ...
    $label = [
        ...
        'tour_scene' => 'tour stop', 'waypoint' => 'campus waypoint',
    ...
    // Much more varied opening sentences based on type and context
    $sentenceStarters = [
        "Step inside {$name}",
        "Visit {$name}",
        "Explore {$name}",
    ...
    $institutionPhrases = [
        "on the {$institution} campus",
        "within {$institution}",
        "at {$institution}",
        "part of {$institution}",
    ];
    
    // Build varied opening sentence
    $starter = $sentenceStarters[array_rand($sentenceStarters)];
    $desc = $descriptionStarters[array_rand($descriptionStarters)];
    
    if ($location !== '' && $institution !== '') {
        $loc = $locationPhrases[array_rand($locationPhrases)];
        $inst = $institutionPhrases[array_rand($institutionPhrases)];
        $sentences[] = "{$starter}, {$desc} {$loc} {$inst}.";
    } elseif ($location !== '') {
        $loc = $locationPhrases[array_rand($locationPhrases)];
        $sentences[] = "{$starter}, {$desc} {$loc}.";
    } elseif ($institution !== '') {
        $inst = $institutionPhrases[array_rand($institutionPhrases)];
        $sentences[] = "{$starter}, {$desc} {$inst}.";
    } else {
        $sentences[] = "{$starter}, {$desc} in the campus area.";
    }

    // Much more varied middle sentences
    if ($facilities !== '') {
        $midTemplates = [
            "This {$label} features {$facilities}, making it perfect for {$purpose}.",
            "You'll find {$facilities} here, designed to support {$purpose}.",
            "The space includes {$facilities}, ideal for {$purpose}.",
            "With {$facilities} available, this area serves {$purpose} well.",
            "This location offers {$facilities}, creating a great environment for {$purpose}.",
            "Equipped with {$facilities}, it's optimized for {$purpose}.",
        ];
        $sentences[] = $midTemplates[array_rand($midTemplates)];
    } elseif ($details !== '') {
        $midTemplates = [
            "Its {$details} design makes this {$label} excellent for {$purpose}.",
            "The {$details} layout works well for {$purpose}.",
            "Designed with {$details} in mind, this space supports {$purpose}.",
            "Featuring {$details}, it provides an ideal setting for {$purpose}.",
            "The {$details} characteristics make it suitable for {$purpose}.",
        ];
        $sentences[] = $midTemplates[array_rand($midTemplates)];
    } elseif ($purpose !== '') {
        $midTemplates = [
            "This {$label} is dedicated to {$purpose} for the campus community.",
            "Students and visitors use this space for {$purpose}.",
            "The area serves {$purpose} needs on campus.",
            "It's designed to facilitate {$purpose} activities.",
            "This location supports {$purpose} for daily campus operations.",
        ];
        $sentences[] = $midTemplates[array_rand($midTemplates)];
    }

    // Much more varied closing sentences
    $closingTemplates = [
        "Navigate to this spot using the campus map or see it in the 360° tour.",
        "Find it easily on the floor plan or explore virtually through the tour.",
        "Use the interactive map to locate this {$label} or take the virtual tour.",
        "This {$label} is accessible via the campus navigation system and featured in the tour.",
        "Visit this location in person or experience it through our 360° virtual tour.",
        "Access this {$label} using the campus map or discover it in the virtual walkthrough.",
        "The navigation system will guide you here, or you can explore it online.",
    ];
    $sentences[] = $closingTemplates[array_rand($closingTemplates)];

    if ($context !== '') {
        $contextTemplates = [
            ucfirst(rtrim(trim(strip_tags($context)), '.')) . '.',
            "Note: " . ucfirst(rtrim(trim(strip_tags($context)), '.')) . '.',
            "Additional info: " . ucfirst(rtrim(trim(strip_tags($context)), '.')) . '.',
        ];
        $sentences[] = $contextTemplates[array_rand($contextTemplates)];
    }
...
/**
 * Resolve a stored media/.mind path to an absolute filesystem path.
 *
 * Stored paths are inconsistent across the app: most images are project-relative
 * ("organizations/<org>/assets/..."), while some legacy .mind paths are
 * org-relative ("assets/ar_targets/minds/..."). Try both forms so downloads,
 * deletes and runners never silently fail on a missing file.
 */
function media_abs_path(string $path, string $orgFolder = ''): string
{
    $path = ltrim(normalize_media_path($path), '/');
    if ($path === '') {
        return '';
    }
    $candidates = [ROOT_PATH . '/' . $path];
    if ($orgFolder !== '') {
        $candidates[] = ROOT_PATH . '/' . trim(str_replace('\\', '/', $orgFolder), '/') . '/' . $path;
    }
...
/**
 * Repair an existing image using OpenRouter AI (Riverflow V2.5 Fast)
 * NOTE: This function repairs/edits EXISTING images (e.g., stitched panoramas),
 * NOT generate new images from scratch. Used in AI stitch to fix damaged elements.
 */
function repair_image_with_openrouter(string $imagePath, string $prompt): ?string
{
    $apiKey = env('OPENROUTER_IMAGE_KEY', env('OPENROUTER_API_KEY', ''));
    if ($apiKey === '') {
        return null;
    }
    ...
    // Use Riverflow V2.5 Fast for image repair as suggested
    $payload = [
    ...
    // For image editing with reference - DISABLED for now due to API format issues
    // OpenRouter Riverflow API has strict format requirements for input_references
    // Future: Enable once correct format is determined
    // if (str_contains($prompt, 'repair') || str_contains($prompt, 'fix') || str_contains($prompt, 'remove')) {
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        ...
        'X-Title: Innovatech PH Image Repair'
    ]);
    ...
    if (isset($result['data'][0]['b64_json'])) {
        $outputPath = dirname($imagePath) . '/repaired_' . basename($imagePath);
    ...
    if (isset($result['error']['message'])) {
        error_log('OpenRouter API error: ' . $result['error']['message']);
    }
