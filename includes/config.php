...
define('BASE_URL', (function () {
    ...
    // Strategy 1: Use DOCUMENT_ROOT (safest for Apache/Nginx and php -S)
    $docRoot = rtrim(str_replace('\\', '/', $_SERVER['DOCUMENT_ROOT'] ?? ''), '/');
    if ($docRoot !== '' && str_starts_with($rootBase, $docRoot)) {
        $prefix = substr($rootBase, strlen($docRoot));
    } else {
        // Strategy 2: Fallback to SCRIPT_NAME / SCRIPT_FILENAME matching
        if ($scriptFile !== '' && str_starts_with($scriptFile, $rootBase)) {
            ...
            if ($trail !== '' && str_ends_with($scriptName, $trail)) {
                $prefix = substr($scriptName, 0, -strlen($trail));
            }
        }
    }
    
    // Safety net: if the prefix leaked a physical absolute path, reset it
    if (preg_match('#^/(?:var|usr|opt|home|etc|Users)/#i', $prefix) || preg_match('#^[a-zA-Z]:/#', $prefix)) {
        $prefix = '';
    }
