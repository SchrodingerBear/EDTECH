 ...
 * Locations are bindable to a building AND a floor plan level.
 * ?fp=<floor_plan_id> lists the rooms + areas bound to that floor plan.
 */
$pageTitle = 'Locations';
...
$backFp = (int) ($_GET['fp'] ?? 0);

$buildings = crud()->select('buildings', 'id, name', ['institution_id' => $iid, 'deleted_at' => ['IS', null]], 'ORDER BY name');
$floorPlans = crud()->raw(
    "SELECT fp.id, fp.title, fp.building_id, b.name AS building_name FROM floor_plans fp
      LEFT JOIN buildings b ON b.id = fp.building_id
     WHERE fp.institution_id = :iid AND fp.deleted_at IS NULL
     ORDER BY COALESCE(b.name, 'zzz'), fp.title",
    [':iid' => $iid]
)->fetchAll();
$fpBuilding = 0;
if ($backFp) {
    foreach ($floorPlans as $fpRow) {
        if ((int) $fpRow['id'] === $backFp) { $fpBuilding = (int) $fpRow['building_id']; break; }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $floorPlanId = (int) ($_POST['floor_plan_id'] ?? 0) ?: null;
    $back = $backFp ? 'admin/institution/locations?fp=' . $backFp : 'admin/institution/locations';
    try {
        if ($action === 'loc-create' || $action === 'loc-edit') {
            ...
            if ($action === 'loc-create') {
                if ($type === 'room') {
                    crud()->insert('rooms', [
                        'institution_id' => $iid, 'building_id' => $buildingId, 'floor_plan_id' => $floorPlanId, 'name' => $name,
                    ...
                    crud()->insert('campus_areas', [
                        'institution_id' => $iid, 'building_id' => $buildingId, 'floor_plan_id' => $floorPlanId,
                ...
                if ($type === 'room') {
                    $updates = ['name' => $name, 'building_id' => $buildingId, 'floor_plan_id' => $floorPlanId, 'code' => trim($_POST['code'] ?? '') ?: null, 'floor_label' => trim($_POST['floor_label'] ?? '') ?: null, 'room_type' => $category ?: null, 'capacity' => (int) ($_POST['capacity'] ?? 0) ?: null];
                    ...
                    crud()->update('campus_areas', [
                        'name' => $name, 'building_id' => $buildingId, 'floor_plan_id' => $floorPlanId,
        ...
        if ($action === 'loc-delete') {
            if ($type === 'room') {
                ...
                crud()->raw('UPDATE campus_areas SET deleted_at=NOW() WHERE id=:id AND institution_id=:iid', ['id' => (int) ($_POST['id'] ?? 0), 'iid' => $iid])->execute();
                flash('success', 'Area location archived.');
            }
        }
    } catch (Throwable $e) {
...
$rows = crud()->raw(
    "SELECT r.id, r.name, r.code, r.floor_label, r.room_type AS category, r.capacity, r.description, r.featured_image_path, r.building_id, r.floor_plan_id, b.name AS building_name, fp.title AS floor_plan_name, 'room' AS kind
       FROM rooms r LEFT JOIN buildings b ON b.id=r.building_id LEFT JOIN floor_plans fp ON fp.id=r.floor_plan_id
      WHERE r.institution_id=:iid1 AND r.deleted_at IS NULL
     UNION ALL
     SELECT a.id, a.name, NULL, NULL, a.area_type, NULL, a.description, NULL, a.building_id, a.floor_plan_id, b.name, fp.title, 'area'
       FROM campus_areas a LEFT JOIN buildings b ON b.id=a.building_id LEFT JOIN floor_plans fp ON fp.id=a.floor_plan_id
      WHERE a.institution_id=:iid2 AND a.deleted_at IS NULL",
    [':iid1' => $iid, ':iid2' => $iid])->fetchAll();

if ($backFp) {
    $rows = array_values(array_filter($rows, function ($r) use ($backFp) { return (int) $r['floor_plan_id'] === $backFp; }));
}
...
$fpTitle = '';
foreach ($floorPlans as $fpRow) { if ((int) $fpRow['id'] === $backFp) { $fpTitle = $fpRow['title']; break; } }
?>
<div class="row g-4">
  <div class="col-12">
    <div class="ia-card">
      <div class="card-head">
        <h3>Locations<?= $backFp ? ' — ' . h($fpTitle) : '' ?></h3>
        <div class="d-flex gap-2 align-items-center">
          <?php if ($backFp): ?>
            <span class="badge badge-live d-inline-flex align-items-center gap-1">
              <?= ia_icon('map', 12) ?> Bound to floor plan
              <a href="<?= url('admin/institution/floor-plan/studio-view.php?id=' . $backFp) ?>" class="text-white text-decoration-underline ms-1">Open Studio</a>
            </span>
          <?php endif; ?>
          <div class="btn-group btn-group-sm" role="group" aria-label="Filter by type">
            <a class="btn <?= $typeFilter === 'all' ? 'btn-grad' : 'btn-outline-ia' ?>" href="<?= url($backFp ? 'admin/institution/locations?fp=' . $backFp : 'admin/institution/locations') ?>">All <?= $roomCount + $areaCount ?></a>
            <a class="btn <?= $typeFilter === 'room' ? 'btn-grad' : 'btn-outline-ia' ?>" href="<?= url($backFp ? 'admin/institution/locations?fp=' . $backFp . '&type=room' : 'admin/institution/locations?type=room') ?>">Rooms <?= $roomCount ?></a>
            <a class="btn <?= $typeFilter === 'area' ? 'btn-grad' : 'btn-outline-ia' ?>" href="<?= url($backFp ? 'admin/institution/locations?fp=' . $backFp . '&type=area' : 'admin/institution/locations?type=area') ?>">Areas <?= $areaCount ?></a>
          </div>
      ...
      <div class="table-responsive">
        <table class="table table-ia">
          <thead>
            <tr>
              <th>Location</th><th>Type</th><th>Category</th><th>Building</th><th>Floor Plan</th><th>Details</th><th class="text-end">Actions</th>
            </tr>
          </thead>
          <tbody>
            <?php foreach ($rows as $r): ?>
              ...
              <tr class="loc-row" data-kind="<?= $r['kind'] ?>" data-name="<?= h(mb_strtolower($r['name'])) ?>">
                ...
                <td class="text-ia-muted">
                  <?php if ($r['floor_plan_name']): ?>
                    <a class="text-decoration-none" href="<?= url('admin/institution/floor-plan/studio-view.php?id=' . (int) $r['floor_plan_id']) ?>" title="Open floor plan studio"><?= h($r['floor_plan_name']) ?></a>
                  <?php else: ?>—<?php endif; ?>
                </td>
                ...
                <td class="text-end">
                  <div class="d-inline-flex gap-1">
                    <button class="btn btn-sm btn-outline-ia" data-bs-toggle="modal" data-bs-target="#location-modal"
                      data-mode="edit" data-kind="<?= $r['kind'] ?>" data-id="<?= (int) $r['id'] ?>" data-name="<?= h($r['name'], ENT_QUOTES) ?>" data-building="<?= (int) $r['building_id'] ?>" data-fp="<?= (int) $r['floor_plan_id'] ?>"
            ...
            <?php endforeach; ?>
            <?php if (!$rows): ?>
              <tr><td colspan="7"><div class="empty-state"><div class="empty-icon"><i class="fas fa-map-marked-alt fa-2x"></i></div><h4>No locations</h4><p>Add buildings first, then create room or area locations bound to a floor plan.</p></div></td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
      ...
      <div class="modal-body d-grid gap-3">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <select class="form-select" name="floor_plan_id" id="loc-fp">
              <option value="">— none —</option>
              <?php foreach ($floorPlans as $fp): ?>
                <option value="<?= (int) $fp['id'] ?>" data-building="<?= (int) $fp['building_id'] ?>"><?= h($fp['title']) ?><?= $fp['building_name'] ? ' · ' . h($fp['building_name']) : '' ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const bldSel = document.getElementById('loc-building')
  const fpSel  = document.getElementById('loc-fp')
  let editKind = null

  function fpBuilding(fpEl) {
    const opt = fpEl.options[fpEl.selectedIndex]
    return opt ? parseInt(opt.dataset.building || '0', 10) : 0
  }

  function syncFpOptions() {
    const bld = parseInt(bldSel.value || '0', 10)
    const cur = fpSel.value
    Array.from(fpSel.options).forEach(opt => {
      const fpb = parseInt(opt.dataset.building || '0', 10)
      opt.disabled = bld !== 0 && opt.value !== '' && fpb !== bld
    })
    if (cur !== '' && fpBuilding(fpSel) !== 0 && parseInt(bldSel.value || '0', 10) !== 0 && fpBuilding(fpSel) !== parseInt(bldSel.value || '0', 10)) {
      fpSel.value = ''
    }
  }

  bldSel.addEventListener('change', syncFpOptions)
  fpSel.addEventListener('change', () => {
    const fpb = fpBuilding(fpSel)
    if (fpb !== 0 && parseInt(bldSel.value || '0', 10) === 0) {
      bldSel.value = String(fpb)
    }
    syncFpOptions()
  })
  ...
  function syncType() {
    ...
    if (editKind === null || editKind === typeSel.value) {
      action.value = (parseInt(document.getElementById('loc-id').value, 10) ? 'loc-edit' : 'loc-create')
    } else {
  ...
  modal.addEventListener('show.bs.modal', (e) => {
    ...
    document.getElementById('loc-building').value = btn.dataset.building || 0
    ...
    // Floor plan: prefer the bound one, else preselect from ?fp= on add
    let fpVal = btn.dataset.fp || 0
    if ((mode === 'create' || !fpVal || fpVal === '0') && window.LOC_INIT_FP) {
      fpVal = window.LOC_INIT_FP
      if (!document.getElementById('loc-building').value || document.getElementById('loc-building').value === '0') {
        const target = Array.from(fpSel.options).find(o => o.value === String(window.LOC_INIT_FP))
        if (target && parseInt(target.dataset.building || '0', 10) !== 0) {
          document.getElementById('loc-building').value = target.dataset.building
        }
      }
    }
    fpSel.value = String(fpVal || 0)
    syncFpOptions()
    syncType()
