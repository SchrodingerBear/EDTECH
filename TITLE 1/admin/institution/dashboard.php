...
$active = 'Overview';
$bodyClass = 'page-institution-dashboard';
...
<div class="d-flex flex-wrap align-items-center gap-3 mb-4">
  <div>
    <h3 class="fw-800 ls-tight"><?= h($inst['name']) ?></h3>
    <p class="mb-0 ia-meta-lg">
      ...
      <div class="card-body">
        <?php if ($startInfo): ?>
          <div class="d-flex align-items-center gap-3">
            <span class="ia-avatar ia-avatar-lg"><?= ia_icon($mode === '360_rotation' ? 'camera' : 'map', 20) ?></span>
            <div>
              <div class="fw-bold"><?= h($startInfo['title']) ?></div>
              <div class="fs-125 text-ia-muted">Visitors start here when they open <?= h($inst['short_name'] ?: $inst['name']) ?></div>
            </div>
          </div>
        <?php else: ?>
        ...
        <table class="table table-ia">
          ...
          <tbody>
            <?php foreach ($recentScenes as $sc): ?>
              <tr>
                <td class="fw-semibold"><?= h($sc['title']) ?><?= $sc['featured_image_path'] ? '' : ' <span class="text-muted">(no featured image)</span>' ?></td>
    ...
    <div class="ia-card mt-4">
      ...
      <div class="card-body card-body-px">
        ...
        <div class="guide-item">
            ...
            <div>Upload <a href="floor-plans">Floor Plans</a> and place markers, or upload panoramas in <a href="tours">360 Tours</a>.</div>
        </div>
        <div class="guide-item">
            <strong>3. Interactive Elements</strong>
            <div>Use the Studio in Floor Plans to add markers, and use the <a href="ar">Augmented Reality</a> tool for campus navigation.</div>
        </div>
        <div class="guide-item">
            <strong>4. Theme & Landing</strong>
            <div>Go to <a href="settings">Theme & Landing</a> to set your starting point, colors, and landing mode.</div>
        </div>
        <div>
            <strong>5. Enhance with AI</strong>
            <div>Optionally use <a href="ai">AI Tools</a> to stitch panoramas or generate facility descriptions.</div>
        </div>
      </div>
    </div>
    ...
    <div class="ia-card mt-4">
      ...
      <div class="card-body">
        <div class="d-flex align-items-center justify-content-between mb-3">
          <div>
            <div class="fw-bold"><?= (int) $inst['is_published'] === 1 ? 'Live for visitors' : 'Unpublished' ?></div>
            <div class="fs-125 text-ia-muted">Controlled by the platform owner</div>
          </div>
          <span class="badge <?= (int) $inst['is_published'] === 1 ? 'badge-live' : 'badge-off' ?>"><?= (int) $inst['is_published'] === 1 ? 'live' : 'draft' ?></span>
        </div>
        <div class="d-flex align-items-center justify-content-between">
          <div>
            <div class="fw-bold">Landscape lock on mobile</div>
            <div class="fs-125 text-ia-muted">AR / 360 landing is fullscreen landscape</div>
          </div>
