...
if ('serviceWorker' in navigator) {
    ...
    if (!isSecureContext && !isLocalhost) {
        console.warn('Service Worker requires HTTPS. Current protocol:', location.protocol);
        console.warn('PWA features including offline mode will not work without HTTPS');
    }
    
    if (isSafari) {
        console.log('Safari detected - service worker support may be limited');
        console.log('Make sure "Service Workers" are enabled in Safari Developer menu');
    }
    
    window.addEventListener('load', async () => {
        try {
            LoadingScreen.updateProgress(10, 'Registering service worker...', 'Enabling offline support');
            console.log('Attempting to register service worker...');
            console.log('Current origin:', location.origin);
            console.log('Secure context:', isSecureContext);
            
            // Try different paths for Safari compatibility
            let registration;
            try {
                // Try root path first
                ...
                console.warn('Failed with /service-worker.js, trying ./service-worker.js');
                try {
                    // Try relative path
                    registration = await navigator.serviceWorker.register('./service-worker.js', {
                        scope: './'
                    ...
                    console.warn('Failed with ./service-worker.js, trying sw.js');
            ...
            LoadingScreen.updateProgress(20, 'Service worker registered', 'Preparing offline mode...');
            ...
            registration.addEventListener('updatefound', () => {
                ...
                newWorker.addEventListener('statechange', () => {
                    if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
                        ...
                        if (window.sphereCapture && window.sphereCapture.cardUI) {
                            window.sphereCapture.cardUI.showUpdatePrompt();
                        }
                    }
                });
            });
            
            // Track if this is the first SW activation
            const isFirstActivation = !navigator.serviceWorker.controller;
            
            // Listen for controller changes but don't auto-reload
            let hasPromptedForReload = false;
            navigator.serviceWorker.addEventListener('controllerchange', () => {
                console.log('Service worker controller changed');
                
                // Skip if this is the first time SW is taking control (not an update)
                if (isFirstActivation) {
                    console.log('First service worker activation - not showing update prompt');
                    return;
                }
                
                // Don't auto-reload - this causes the refresh issue!
                // Only show update prompt once per session for actual updates
                if (!hasPromptedForReload && window.sphereCapture && window.sphereCapture.cardUI) {
                    hasPromptedForReload = true;
                    // Delay prompt to avoid immediate refresh on first load
                    setTimeout(() => {
                        window.sphereCapture.cardUI.showUpdatePrompt();
            ...
            console.error('1. Not on HTTPS (required for service workers)');
            console.error('2. Service worker file not found');
            console.error('3. Syntax error in service worker');
        }
    });
    
    // Check service worker status after 3 seconds
    setTimeout(() => {
        if (navigator.serviceWorker.controller) {
            console.log('✅ Service Worker is controlling the page');
        } else {
            console.warn('⚠️ No Service Worker controlling the page');
            console.warn('Offline functionality will not work');
        }
    }, 3000);
} else {
