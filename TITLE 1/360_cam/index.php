...
<html lang="en">
 <head>
  ...
  <link href="./img/icon-167.png" rel="apple-touch-icon" sizes="167x167"/>
  <link href="./img/icon-180.png" rel="apple-touch-icon" sizes="180x180"/>
  <!-- Favicon -->
  <link href="./img/favicon-32.png" rel="icon" sizes="32x32" type="image/png"/>
  <link href="./img/favicon-16.png" rel="icon" sizes="16x16" type="image/png"/>
  <!-- PWA Manifest -->
 ...
 <body>
  ...
  <!-- App Loading Indicator - Shows immediately on first load -->
  <div id="app-loading">
   <div class="loader-card">
    <!-- Logo -->
    <img alt="Photosphere" src="./img/logo_150x150.png"/>
    <!-- Title -->
    <h2>
     Innovatech 360 Camera
    </h2>
    <!-- Status message -->
    <div id="app-status">
    ...
    <div class="loader-bar">
     ...
     <div id="app-progress-indeterminate">
     </div>
    </div>
    <!-- Sub-status for details -->
    <div id="app-substatus">
     Please wait...
    </div>
   </div>
  </div>
  <!-- 3D Scene -->
  <div aria-hidden="true" id="scene-container">
  </div>
  <!-- Instructions -->
  <div aria-hidden="true" aria-live="polite" id="instructions" role="status">
   <!-- Will be updated by JavaScript -->
  </div>
  <!-- Screen reader announcements (visually hidden) -->
  <div aria-atomic="true" aria-live="polite" class="sr-only" id="sr-announcements" role="status">
  </div>
  <!-- Camera viewport (vertical rectangle) -->
  <div aria-label="Camera viewfinder" id="camera-viewport">
   <div aria-hidden="true" id="alignment-indicator">
   </div>
   <div aria-hidden="true" aria-label="Device level indicator" id="roll-indicator">
    <div id="roll-line-static-left">
    </div>
    <div id="roll-line-moving">
    </div>
    <div id="roll-line-static-right">
    </div>
   </div>
  </div>
  <!-- Hidden elements -->
  <video autoplay="" id="camera-full" muted="" playsinline="">
  </video>
  <canvas id="capture-canvas">
  </canvas>
  <!-- Stitching progress overlay -->
  <div aria-hidden="true" aria-labelledby="stitch-title" aria-modal="true" class="ui-card" id="stitching-overlay" role="dialog">
...
100|      </path>
   ...
   <div class="card-content">
    ...
    <canvas data-thumbnail="./img/360_blank_thumbnail.jpg" height="300" id="preview-canvas" width="600">
    </canvas>
    <div class="action-stack">
     <!-- View and Share buttons side by side -->
     <div class="action-pair">
      <button aria-label="View panorama" id="view-btn" class="action-btn-styled">
       <img alt="View icon" src="./img/view.svg"/>
       View
      </button>
      <button aria-label="Share panorama" id="download-btn" class="action-btn-styled">
       <img alt="Share icon" src="./img/share.svg"/>
       Share
      </button>
     </div>
     <!-- Camera Roll button full width -->
     <button aria-label="View Camera Roll" id="view-camera-roll-btn" class="action-btn-styled" onclick="openGallery()">
      <img alt="Camera roll icon" src="./img/camera-roll.svg"/>
      View Camera Roll
     </button>
  ...
  <div aria-labelledby="home-title" class="ui-card home-card visible" id="start-screen" role="main">
   <div class="card-header">
    <img alt="Photosphere Camera Logo" class="logo logo-glow" src="./img/logo_150x150.png" width="60"/>
    <h1 class="card-title" id="home-title">
     Innovatech 360 Camera
    </h1>
   </div>
   <div class="card-content">
    <div class="home-instructions">
     <div class="step-row">
      ...
      <span>
       Hold your device close to your face, and rotate your body to find the
     ...
     <div class="step-row">
      ...
      <span>
       Images will be automatically captured when aligned
      </span>
     </div>
     <div class="step-row">
      <span class="step-num">
      ...
      <span>
       Watch the progress circle fill as you complete each hotspot
      </span>
     </div>
     <div class="step-row">
      <span class="step-num">
      ...
      <span>
       After capturing all 36 images, tap the checkmark button to continue
      </span>
     </div>
    </div>
    <div class="start-actions">
     <button aria-label="Start capturing photosphere" id="start-btn" onclick="startApp().catch(console.error)">
      <img alt="Camera icon" src="./img/camera.svg"/>
      Start Capturing
     </button>
    </div>
   </div>
   <button aria-label="Open settings menu" class="control-btn settings-btn-start-screen" id="settings-btn" onclick="openSettings()" title="Settings">
    <img alt="Settings icon" src="./img/settings.svg"/>
   </button>
   <button aria-label="View camera roll" class="control-btn gallery-btn-start-screen" id="gallery-btn" onclick="openGallery()" title="View Camera Roll">
    <img alt="Camera roll icon" src="img/camera-roll.svg"/>
   </button>
  </div>
  <!-- Settings Menu Card -->
  ...
  <div aria-hidden="true" class="ui-card" id="info-card">
   ...
   <div class="card-header">
    <h1 class="card-title" id="info-card-title">
     Info &amp; Settings
    </h1>
   </div>
   <div class="card-content" id="info-card-content">
    <!-- Content will be populated dynamically -->
   </div>
  </div>
  <!-- Custom Dialog Card -->
  <div aria-describedby="dialog-message" aria-hidden="true" aria-labelledby="dialog-title" aria-modal="true" class="ui-card dialog-card" id="dialog-card" role="dialog">
   <div>
    <h3 id="dialog-title">
    </h3>
    <p id="dialog-message">
  ...
  <div aria-hidden="true" aria-labelledby="orientation-title" aria-live="assertive" class="ui-card dialog-card" id="orientation-warning-card" role="alert">
   <div>
    <img alt="Rotate Device" src="./img/mobile-rotate.svg"/>
    <h3 id="orientation-title">
     Rotate Your Device
    </h3>
    <p>
     Please rotate your device to vertical (portrait) orientation to continue capturing.
    </p>
   </div>
  </div>
  <div aria-hidden="true" aria-labelledby="camera-roll-title" aria-modal="true" class="ui-card camera-roll-card" id="camera-roll-card" role="dialog">
   ...
   <div class="card-header">
    ...
    <p id="camera-roll-count">
     Loading...
    </p>
   </div>
   <div class="card-content" id="camera-roll-list">
    <!-- Populated by JavaScript from IndexedDB -->
   </div>
  ...
  <div class="bottom-controls">
   ...
   <!-- Capture button with progress circle -->
   <div id="capture-btn-container">
    <svg id="captureProgress" viewbox="0 0 36 36">
     <path d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" id="progressPath" stroke="#4CAF50" stroke-dasharray="100, 100" stroke-dashoffset="100" stroke-width="3">
     </path>
    </svg>
   ...
   <!-- Photosphere Gallery button -->
   <button aria-label="View camera roll" class="control-btn" id="gallery-btn" onclick="openGallery()" title="View Camera Roll">
